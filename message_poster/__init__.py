from mcdreforged.api.types import PluginServerInterface, CommandSource, PlayerCommandSource, ConsoleCommandSource, Info
...
def set_webhook_url(src: CommandSource, ctx: CommandContext):
    if not src.has_permission(4):
        src.reply(f'[{RColor.aqua.mc_code}Message Poster{RColor.white.mc_code}] {RColor.red.mc_code}{LANGS[lang]["permission_denied"]}')
        return
    ...
    file.close()
    src.reply(f'[{RColor.aqua.mc_code}Message Poster{RColor.white.mc_code}] {RColor.green.mc_code}{LANGS[lang]["done"]}')
...
def set_language(src: CommandSource, ctx: CommandContext):
    ...
    if ctx['lang'] not in LANGS.keys():
        src.reply(f'[{RColor.aqua.mc_code}Message Poster{RColor.white.mc_code}] {RColor.red.mc_code}{LANGS[lang]["invalid_language"]}')
        return
